...
export const TrainCar = ({ variant = "middle" }: TrainCarProps) => {
  ...
  return (
    ...
    >
      ...
      {!isFront && (
        <div
          className={`absolute top-2 ${isRear ? "left-3 right-1" : "left-1 right-1"} h-5 flex ${isRear ? "gap-0.5" : isMiddle ? "gap-0.5" : "gap-0"}`}
        >
      ...
      {hasRedLight && (
        <div
          className="absolute top-7 left-2 w-2 h-2 bg-red-500 rounded-full shadow-lg animate-pulse"
